...
const API = {
  ...
  */
  ...
  async _registerOrUnregisterSenseData(senseData:SenseData, sensesDataList:SenseData[], valueComment:string, unregister:boolean){
    if(!senseData.id){
      if(unregister){
    ...
    if(unregister){
      sensesDataList = sensesDataList.filter(function(el) { return el.id != senseData.id; });
    }else{
